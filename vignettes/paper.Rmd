...
- The previously used a approach contained a methodological flaw which led to the impact of ending BCG vaccination being greatly underestimated.
...
-	This study reassesses a key piece of the quantitative evidence used to motivate the change in BCG vaccination policy in 2005, correcting a methodological flaw in the original model.
...
We fitted this parameter using least squares to the original estimates of the total notifications due to ending the scheme under several scenarios, for several years. We validated the fitted model by comparing the results with those from the original implementation using the mean absolute percentage error, normalised by the original estimate, as the performance metric. This approach is only valid when using the original approach from [@Sutherland1989] and so is only used here for validation.
